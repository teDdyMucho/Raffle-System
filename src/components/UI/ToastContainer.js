  ...
  return (
    <>
      ...
      <div className="fixed bottom-4 right-4 sm:left-auto sm:right-4 left-4 z-[100] space-y-3 max-w-full sm:max-w-sm">
        {toasts
          .filter(t => (t.position || 'bottom-right') === 'bottom-right')
          .map(t => {
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={`shrink-0 mt-0.5 rounded-full p-1.5 ${theme.iconBg}`}>
                    {theme.icon}
                  </div>
                ...
                {/* Progress bar */}
                <div
                  className={`absolute left-0 bottom-0 h-0.5 ${theme.bar} animate-[progressLinear_var(--dur)_linear_forwards]`}
                  style={{ width: '100%', '--dur': `${t.duration || 3000}ms` }}
                />
              </div>
            );
          })}
            ...
            .map(t => {
              ...
              return (
                ...
                >
                  ...
                  {/* Progress bar */}
                  <div
                    className={`absolute left-0 bottom-0 h-1 ${theme.bar} animate-[progressLinear_var(--dur)_linear_forwards]`}
                    style={{ width: '100%', '--dur': `${t.duration || 3000}ms` }}
                  />
                </div>
              );
